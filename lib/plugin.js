...
exports.register = function (server, opts, next) {
    ...
    Promise.all(_.map(adapters, (adapter)=>adapter.connect()))
        .then(()=> {
            ...
            const get = function (schema, resultFn) {
                ...
                return _.merge(routes.get(schema), {
                    handler: (req, reply) => {
                        ...
                        let resultPromise;
                        if(resultFn){
                            resultPromise = resultFn(req, {filter, skip, limit, sort, fields})
                        } else {
                            resultPromise = adapter.find(schema.type, filter, skip, limit, sort, fields)
                        }
                        reply(resultPromise.then(data => {
                            return {data}
                        }))
                    }
                })
            ...
            const getById = function (schema, resultFn) {
                onRouteRegister(schema)
                ...
                var getByIdRoute = _.merge(routes.getById(schema), {
                    handler: (req, reply) => {
                        ...
                        const fields = extractFields(query, schema);

                        let resultPromise;
                        if(resultFn){
                            resultPromise = resultFn(req, {id, fields})
                        } else {
                            resultPromise = adapter.findById(schema.type, id, fields)
                              .then(found => {
                                if(!found){
            ...
            const post = function (schema, resultFn) {
                onRouteRegister(schema)
                return _.merge(routes.post(schema), {
                    handler: (req, reply) => {
                      ...
                      let resultPromise;
                      if(resultFn){
                          resultPromise = resultFn(req, {payload})
                      } else {
                          resultPromise = adapter.create(schema.type, payload.data).catch(err => {
            ...
            const patch = function (schema, resultFn) {
                onRouteRegister(schema)
                const patchRoute = _.merge(routes.patch(schema), {
                    handler: (req, reply) => {
                      ...
                      let resultPromise;
                      if(resultFn){
                          resultPromise = resultFn(req, {id, payload})
                      } else {
                          resultPromise = adapter.patch(schema.type, id, payload.data).then(found=> {
                              if (!found) {
                                  return Promise.reject(Boom.notFound());
                              }
                              return found;
            ...
            const del = function (schema, resultFn) {
                onRouteRegister(schema)
                return _.merge(routes.delete(schema), {
                    handler: (req, reply) => {
                        ...
                        if(resultFn){
                          ...
                          resultPromise = adapter.delete(schema.type, id);
                        }
                        reply(resultPromise).code(204)
                    }
                })
            }
            ...
            const createModel = function (schema) {
                onRouteRegister(schema)
            ...
            server.ext('onRequest', (request, reply) => {
                if (request.method === 'get') {
                    ...
                    if (match) {
                        ...
                        const isRelationship = _.includes(_.keys(schema.relationships), relationshipName)
                        if (isRelationship) {
                            const url = '/' + type + '/' + parentId;
                            //TODO: for inject better pass credentials to avoid doing auth again
