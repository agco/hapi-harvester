...
exports.register = function (server, opts, next) {
    ...
    const get = function (schema) {
        ...
        return _.merge(routes.get(schema), {
            handler: (req, reply) => {
                ...
                reply(adapter.find(schema.type, filter, skip, limit, sort, fields))
            }
        })
    }
    ...
    const getById = function (schema) {
        ...
        return _.merge(routes.getById(schema), {
            handler: (req, reply) => {
                adapter.findById(schema.type, req.params.id, extractFields(req, schema)).then((found)=>{
                    if (found) {
                        reply(found)
                    } else {
                        reply(Boom.notFound())
    ...
    const post = function (schema) {
        ...
        return _.merge(routes.post(schema), {
            handler: (req, reply) => {
                reply(adapter.create(schema.type, req.payload.data)).code(201)
            }
        })
    }
    ...
    const patch = function (schema) {
        ...
        return _.merge(routes.patch(schema), {
            handler: (req, reply) => {
                adapter.update(schema.type, req.params.id, req.payload.data).then((found)=>{
                    if (found) {
                        reply(found)
                    } else {
                        reply(Boom.notFound())
