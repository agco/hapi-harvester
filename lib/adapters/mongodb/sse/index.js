...
module.exports = function (mongodbOplogUrl, options) {
    ...
    const streamChanges = function (resources, verbs, lastEventId) {
      console.log(`Internal streamChanges() resources: ${JSON.stringify(resources)}, verbs: ${JSON.stringify(verbs)} lastEventId: ${lastEventId}`);
        ...
        return Promise.resolve()
            .then(()=> {
                if (lastEventId) {
                    ...
                    const isValidTS = _.every(tsSplit, function (ts) {
                        return !isNaN(ts)
                    })
                    if (!isValidTS) {
                ...
                return getQuery(verbs, lastEventId, regex)
                    .then(function (query) {
                      ...
                      console.log(`Internal getQuery() result: ${JSON.stringify(query)}`);
                                ...
                                (d) => {
                                    return RxNode.fromStream(d.stream)
                                        .map((chunk)=> {
                                            ...
                                            if(!firstChunkLogged){
                                              console.log(`Internal first chunk: ${JSON.stringify(result)}`)
