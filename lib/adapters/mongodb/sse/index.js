...
module.exports = function (mongodbOplogUrl, options) {
    ...
    const connect = function () {
        return connection.connect(mongodbOplogUrl, _.merge({server: {maxPoolSize: 100}}, options))
            .then((dbConnected)=> {
                db = dbConnected
            })
    }
    ...
    const streamChanges = function (resources, verbs, lastEventId) {
        ...
        if (lastEventId) {
            ...
            if (!isValidTS) {
                throw Boom.badRequest(`the last-event-id provided is not valid`, lastEventId)
            }
        }
