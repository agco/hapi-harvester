...
function sse(config) {
    ...
    config.db = mongojs(config.context.plugins.harvester.adapter.options.oplogConnectionString)
...
function getMissingResources(config, resourceNames) {
    var harvesterResourceNames = config.singleResourceName || _.keys(config.context.plugins.harvester.schemas)
