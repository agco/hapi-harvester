...
function sse(config) {
    ...
    config.db = mongoose.createConnection(config.context.plugins['hapi-harvester'].adapter.options.oplogConnectionString, {
        server: {
            poolSize: 100,
            keepAlive: 1
        }
    });
    config.db.on('error', (e) => {
        console.error(e)
    })
    ...
    var regex = new RegExp('.*\\.(' + routeNames.join('|') + ')', 'i')
        ...
        getQuery(config, req, regex)
            .then(function (query) {
                ...
                const oplogCollection = config.db.collection('oplog.rs')
                const stream = oplogCollection.find(query, options).stream()

                var eventStream = Rx.Observable.using(
                    () => {
                        return new DisposableOplogStream(stream, reply)
                    },
                    (d) => {
                        return RxNode.fromStream(d.stream)
                            .map((chunk)=> {
                                return {
                                    ...
                                    event: getEventName(routeNames[0], chunk),
                                    data: getData(chunk)
                                }
                            })
                            .filter((event) => {
                ...
                Rx.Observable.merge(eventStream, tickerStream)
                    .subscribe(
                        (event) => {
                            reply.event(event)
                        },
