...
function handler(config, req, reply) {
    console.log(`Request to SSE. path: ${req.path}, Query: ${JSON.stringify(req.query)} Headers: ${JSON.stringify(req.headers)}`);
    Promise.resolve()
        .then(() => {
...
function getFilters(req) {
    ...
    return _.chain(req.query)
        .map(function (item, key) {
            if (!_.includes(['limit', 'sort', 'offset', 'resources'], key)) {
                var filter = {}
                filter[key] = item
                return filter
            }
